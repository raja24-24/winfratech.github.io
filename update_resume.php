...
if(isset($_POST['submit_resume_form'])){
	...
	$resume_id = addslashes($_POST['resume_id']);
	$user_logo = $file_name;
	...
	/*$sql = "INSERT INTO `resumes` (`user_id`,`first_name`,`last_name`,`email`,`phone`,`gender`,`birthdate`,`mother_tongue`,`languages_known`,`nationality`,`marital_status`,`profile_photo`,`address_1`,`state`,`address_2`,`postcode`,`city`,`country`,`academic_details`,`project_details`,`technical_skills`,`key_skills`,`personality_traits`,`achivements`,`workshops_seminars`,`created_by`,`created_at`) 
	VALUES ('$user_id','$first_name','$last_name','$email','$phone','$gender','$birthdate','$mother_tongue','$languages_known','$nationality','$marital_status','$profile_photo','$address_1','$state','$address_2','$postcode','$city','$country','$academic_details','$project_details','$technical_skills','$key_skills','$personality_traits','$achivements','$workshops_seminars','$user_id','$created_at')";*/

	$sql = "UPDATE `resumes` SET  `user_logo`='$user_logo',`first_name`='$first_name', `last_name`='$last_name', `email`='$email', `birthdate`='$birthdate', `mother_tongue`='$mother_tongue', `languages_known`='$languages_known', `nationality`='$nationality', `marital_status`='$marital_status', `profile_photo`='$profile_photo' , `address_1`='$address_1' ,`state`='$state' ,`address_2`='$address_2' ,`postcode`='$postcode' ,`city`='$city' ,`country`='$country' ,`academic_details`='$academic_details', `project_details`='$project_details', `technical_skills`='$technical_skills', `key_skills`='$key_skills', `personality_traits`='$personality_traits', `achivements`='$achivements', `workshops_seminars`='$workshops_seminars'  WHERE `resume_id`='$resume_id' ";

	//echo "$sql";exit;

	if (mysqli_query($conn, $sql)) {
  		echo "Resume has been Updated Successfully!";
